  ...
  return (
    <div className="p-3 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="mb-4 sm:mb-6 lg:mb-8">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-brand-ink">
          Hola <span className="text-brand-green">{user?.name ?? ""}</span>,
        </h1>
        <p className="text-brand-ink/50 text-sm sm:text-base">
        ...
        {stats.map((s) => {
          ...
          return (
            <Link
              key={s.label}
              href={s.href}
              className="card p-3 sm:p-5 hover:border-brand-green/60 hover:shadow-glow transition min-w-0"
            >
              <div className="flex items-center justify-between mb-1.5 sm:mb-2 gap-2 min-w-0">
                <span className="text-[10px] sm:text-xs uppercase tracking-wider text-brand-ink/50 truncate">
                  {s.label}
                </span>
                <Icon size={16} className="text-brand-green shrink-0" />
              </div>
              <div className="text-xl sm:text-3xl font-bold text-brand-ink">
                {s.value}
              </div>
              <div className="text-[11px] sm:text-xs text-brand-ink/50 mt-1 break-words">
                {s.sub}
              </div>
            </Link>
          );
